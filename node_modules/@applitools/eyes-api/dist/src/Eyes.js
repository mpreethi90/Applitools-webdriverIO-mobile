...
class Eyes {
    constructor(runnerOrConfig, config) {
        ...
        if (utils.types.instanceOf(runnerOrConfig, Runners_1.EyesRunner)) {
            ...
            this._config = new Configuration_1.ConfigurationData(runnerOrConfig);
        }
    ...
    async open(driver, configOrAppName, testName, viewportSize, sessionType) {
        ...
        if (utils.types.isEnumValue(sessionType, SessionType_1.SessionType))
    ...
    async close(throwErr = true) {
        ...
        if (!this.isOpen)
            throw new EyesError_1.EyesError('Eyes not open');
        const results = new TestResults_1.TestResultsData(await this._eyes.close(), options => this._spec.deleteTest(Object.assign(Object.assign({}, options), { serverUrl: this._config.serverUrl, apiKey: this._config.apiKey, proxy: this._config.proxy })));
        this._eyes = null;
        if (throwErr) {
            if (results.status === TestResultsStatus_1.TestResultsStatus.Unresolved) {
                if (results.isNew)
                    throw new NewTestError_1.NewTestError(results);
                else
                    throw new DiffsFoundError_1.DiffsFoundError(results);
            }
            else if (results.status === TestResultsStatus_1.TestResultsStatus.Failed) {
                throw new TestFailedError_1.TestFailedError(results);
            }
        }
        return results;
    }
    ...
    async abort() {
        if (!this.isOpen || this._config.isDisabled)
            return null;
        const results = new TestResults_1.TestResultsData(await this._eyes.abort(), options => this._spec.deleteTest(Object.assign(Object.assign({}, options), { serverUrl: this._config.serverUrl, apiKey: this._config.apiKey, proxy: this._config.proxy })));
        this._eyes = null;
        return results;
    }
    /** @deprecated */
    ...
    async getViewportSize() {
        return this._config.getViewportSize() || new RectangleSize_1.RectangleSizeData(await this._spec.getViewportSize(this._driver));
    }
